...
export enum Statuses {
     ...
     *
     * The response `417 Expectation Failed` indicates the request should not
     * be continued.
     */
    Continue = 100,
     ...
     * options, to list files with different filename extensions, or to
     ...
     * functionality of a `303 See Other`.
     *
     ...
     * method.
     *
     * When received in response to a `POST` (or `PUT`/`DELETE`), the client
     ...
     * allow the HTTP method to change.
     *
     ...
     * form of digital cash or micro payment scheme, but that has not happened,
     ...
     *
     * Often the result of too much data being encoded as a query-string of a
     ...
     * support.
     *
     * For example, the client uploads an image as `image/svg+xml`, but the
     ...
     *
     * The RFC specifies this code should be returned by teapots requested to
     * brew coffee.
     *
     * This HTTP status is used as an Easter egg in some websites, including
     * Google.com.
     */
    IAmATeapot = 418,
     ...
     *
     * Intended to prevent
     * > the 'lost update' problem, where a client `GET`s a resource's state,
     * > modifies it, and `PUT`s it back to the server, when meanwhile a third
