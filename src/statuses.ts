...
export enum Statuses {
     ...
     * @since HTTP/1.1
     *
     *   The server is a transforming proxy that received a `200 OK` from its origin, but is
     ...
     * or micro payment scheme, but that has not happened, and this code is not usually used.
     *
     ...
     *
     * Most use cases do not require clients and search engines to purge the resource, and a `404
     ...
     *
     * Often the result of too much data being encoded as a query-string of a `GET` request, in
     ...
     * Intended to prevent
     *
     * The 'lost update' problem, where a client `GET`s a resource's state, modifies it, and `PUT`s
